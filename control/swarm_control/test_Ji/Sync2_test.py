...
def initValues(autoMat,syncMat,numOfAgents,numOfGroups,regionCenters,offsetVector):
	...
	print "goalPose=",goalPose,"V"

	return (numOfStates,memory,goalPose,offsetGoal,advance)

def isReady(autoMat,syncMat,numOfAgents,numOfGroups,numOfStates,memory,goalPose,offsetGoal,currentPose,loopingIndex,advance,regionCenters,offsetVector,regionParking):
	# lists of 1-by-N matrices
	#numOfGroups = len(autoMat)
	print "goalPose=",goalPose,"A"
	...
	goalPoseTmp = np.zeros([2,numOfAgents])
	goalPoseTmp = np.matrix(goalPose)
	for i in range(numOfAgents):
		goalPoseTmp[:,i] = goalPose[:,i]

	print "goalPose=",goalPose,"B"
	...
	for i in range(numOfAgents):
		...
		print "goalPose=",goalPose,"C"

		if autoMat[groupId][0,memory[0,i]]-1 == 0:
			goalPoseTmp[:,i] = regionParking[0][:,i+1]
		elif autoMat[groupId][0,memory[0,i]]-1 == 1:
			goalPoseTmp[:,i] = regionParking[1][:,i+1]
		elif autoMat[groupId][0,memory[0,i]]-1 == 2:
			goalPoseTmp[:,i] = regionParking[2][:,i]
		else:
			print "."

		print "goalPose[:,i]=",goalPose[:,i]
		print "goalPoseTmp[:,i]=",goalPoseTmp[:,i]
		    

	return (memory,goalPose,goalPoseTmp,offsetGoal,advance)
