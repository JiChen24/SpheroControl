...
controlServer = wifiComm.wifiComm()
controlServer._init_server()
...
if __name__=="__main__":
    ...
    for i in range(Num):
        ...
        sphero_code[i] = str(sys.argv[i+1]).upper()
    ...
    regionCenters = numpy.matrix([[0,2.0,-0.6,0.6,-0.8,0,1.2,-2.2],[0,0,2.5,2.5,-2.5,-2.5,-2.5,0]])
    tol = 0.1
    offsetVector = numpy.matrix([[0.5+tol,0.3+tol,0.3+tol,0.3+tol,0.2+tol,0.2+tol,0.2+tol,0.3+tol],[0.5+tol,0.5+tol,0.3+tol,0.3+tol,0.4+tol,0.4+tol,0.4+tol,0.6+tol]])
    ...
    for i in range(Num):
        ...
        while (time.time() - startCalib < 1):
            ...
            controlServer.sendCmdPacket(sphero_code[i],'v',[100,0])
            rate.sleep()
        ...
        pub[i].publish(vel_msg)
        controlServer.sendCmdPacket(sphero_code[i],'v',[0,0])
        rate.sleep()
...
#index of first goal is 0, last is num_waypoints-1
    ...
    while (time.time()-start<20): # not at goal point
        ...
        for i in range(Num):
            ...
            controlServer.sendCmdPacket(sphero_code[i],'v',vCmd)
            rate.sleep()
