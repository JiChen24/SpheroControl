...
if __name__=="__main__":
    ...
    tol = 0.1
    offsetVector = numpy.matrix([[0.8+tol,0.8+tol,0.45+tol,0.45+tol,0.3+tol,0.3+tol,0.3+tol,0.5+tol],[1.1+tol,1.1+tol,0.5+tol,0.5+tol,0.5+tol,0.5+tol,0.5+tol,0.7+tol]])

    regionParking = [[]]*3
    regionParking[0] = numpy.matrix([[-0.6,0.0,0.6,-0.6,0.0,0.6,-0.6,0.0,0.6],[1.0,1.0,1.0,0.2,0.2,0.2,-0.8,-0.8,-0.8]])
    regionParking[1] = numpy.matrix([[1.1,1.8,2.5,1.1,1.8,2.5,1.1,1.8,2.5],[1.0,1.0,1.0,0.2,0.2,0.2,-0.8,-0.8,-0.8]])
    regionParking[2] = numpy.matrix([[-0.5,0,-0.5,0,-0.5,0,-0.5,0,-0.2],[3.0,3.0,2.2,2.2,3.0,3.0,2.2,2.2,2.5]])
    
    (numOfStates,memory,goalPoseForSync,offsetGoal,advance) = Sync2_test.initValues(autoMat,syncMat,numAll,numOfGroups,regionCenters,offsetVector)
...
#index of first goal is 0, last is num_waypoints-1
    ...
    while (time.time()-start<10): # not at goal point

    	time_tmp = time.time()

    	(memoryNew,goalPoseForSyncNew,goalPoseForSyncTmp,offsetGoalNew,advanceNew) = Sync2_test.isReady(autoMat,syncMat,numAll,numOfGroups,numOfStates,memory,goalPoseForSync,offsetGoal,\
            															poseAll,loopingIndex,advance,regionCenters,offsetVector,regionParking)
    	...
    	for j in range(Num):
            loc_goal[j][0] = goalPoseForSyncTmp[0,j+Diff]
            loc_goal[j][1] = goalPoseForSyncTmp[1,j+Diff]
        ...
        for i in range(Num):
            ...
            if dist2goal[i] > 0.05:
                ...
                try:
                    ...
                    vel_mag = (v_vect_V_actual[1]**2+v_vect_V_actual[0]**2)**0.5*100+55
                ...
                vel_msg = geometry_msgs.msg.Twist(geometry_msgs.msg.Vector3(10,10,0), \
                                                geometry_msgs.msg.Vector3(0,0,0))
        ...
        print "timeForLoop is", time.time()-time_tmp
