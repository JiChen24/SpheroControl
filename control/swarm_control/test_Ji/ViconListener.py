...
def plot_init_():
	...
	MAP = numpy.matrix([[-wallRange,-wallRange,wallRange,-wallRange],[wallRange,-wallRange,wallRange,wallRange+0.6],\
						...
						[-3.0,2.2,-1.4,2.2],[-3.0,-1.6,-1.4,-1.6],[-1.0,3.6,-1.0,2.0],[1.8,3.6,1.8,2.0],\
						[-0.8,-3.0,-0.8,-1.4],[0.2,-3.0,0.2,-1.4],[1.2,-3.0,1.2,-1.4],\
						[1.4,-3.0,1.4,-1.4],[2.4,-3.0,2.4,-1.4],[0.4,3.6,0.4,2.0]])
	nWalls = MAP.shape[0]

	door = numpy.matrix([[-1.0,2.0,1.8,2.0],[-1.4,2.2,-1.4,-1.6],[-0.8,-1.4,1.2,-1.4],[1.4,-1.4,2.4,-1.4]])
	nDoors = door.shape[0]
	...
	regionCenters = numpy.matrix([[0,2.3,-0.3,1.1,-0.3,0.7,2.1,-2.2],[0.2,0.2,3.1,3.1,-2.6,-2.6,-2.6,0.3]])
	tol = 0.1
	offsetVector = numpy.matrix([[0.8+tol,1.3+tol,0.45+tol,0.45+tol,0.3+tol,0.3+tol,0.3+tol,0.5+tol],[1.2+tol,1.1+tol,0.9+tol,0.9+tol,0.9+tol,0.9+tol,0.9+tol,1.5+tol]])
